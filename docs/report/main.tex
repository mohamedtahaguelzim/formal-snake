...
\abstract{
    The Snake game, while considered trivial software, presents a complex environment and large state spaces
    which makes it an excellent candidate for a formal verification case study.
    In this project, we model the game as a state machine and verify the correctness of
    ...
    Our results demonstrate that modern SMT-based verification tools can effectively guarantee the
    absence of illegal states in dynamic game environments.
...
Formal verification is traditionally associated with safety-critical systems; however, applying these
methods to game development offers a unique opportunity to study state-based reasoning
...
dying if it collides with the walls or itself. While traditionally a real-time game, our
implementation focuses on a turn-based version, meaning the game state updates only when the player
provides input (although the game is playable in real-time too).
...
\begin{itemize}
    \item We formalize Snake as a finite state machine in Scala, separating control flow from the
        game logic.
    \item We define inductive invariants, encompassing geometric bounds and topological consistency.
    \item We provide machine-checked proofs that the game transition function preserves these
        invariants and that the food generation algorithm correct.
\end{itemize}


\section{Model and Game Logic}
The game is implemented as a finite state machine, the implementation is available on 
...
\end{minted}
where the \texttt{status} field determines the high-level mode of the FSM described in
Figure~\ref{fig:status}.
The game state is represented using immutable data structures, which simplifies formal reasoning
about state transitions by making each transition explicit and side-effect free.
The core verification focuses on the \mintinline{scala}{Playing} status, where the logic is
non-trivial.

The core logic resides in the function \mintinline{scala}{processGameTick} which advances the game
state by one tick while in a \mintinline{scala}{Playing} status.
...
\mintinline{scala}{nextHead :: oldSnake}; otherwise it is \newline
\mintinline{scala}{nextHead :: withoutLast(oldSnake)} where \texttt{withoutLast} removes the last
element of the list.
...
\mintinline{scala}{validTransition} which describes valid state transitions.
These conditions ensure that the game remains consistent:
\begin{itemize}
...
show that \newline
\mintinline{scala}{processGameTick} preserves the invariant when the game does not end.
...
\mintinline{scala}{validTransition}.
...
The formalization was successfully verified by Stainless. On a standard modern CPU, the full
verification suite completes in approximately 3 seconds without caching. In the GitHub CI/CD
pipeline, the verification takes approximately 10 seconds.

The verified code-base consists of approximately 400 lines of Scala code, split roughly in one third
for the game implementation and two thirds for the proof infrastructure.
A significant portion of the effort was dedicated to auxiliary lemmas.
This demonstrates a typical ratio in formal verification where the time spent on proofs and the
proof code size exceeds the implementation's.


\section{Conclusion}
In this project, we demonstrated that formal verification methods can be effectively applied to
state-based games like Snake. By modeling the game as a finite state machine and defining
...
concerning list manipulations such as contiguity, subsequence preservation and set theory
...
This project also highlights the limitations of automated verification tools, as several non-trivial
properties, particularly those involving list manipulation and grid reasoning, required carefully
designed auxiliary lemmas.


\section{Further Work}
While this project establishes the correctness of the core Snake game logic, several meaningful
extensions could be explored in future work.

\paragraph{Stronger Properties and Liveness}
Our verification focuses primarily on safety properties, ensuring that the game never reaches an
invalid state. A natural extension would be to reason about liveness properties. For example,
one could prove that from any valid \mintinline{scala}{Playing} state, the game eventually reaches a
terminal state (\mintinline{scala}{GameOver} or \mintinline{scala}{GameWon}) under reasonable
assumptions on player input. Another direction would be to formally characterize determinism, i.e.,
prove that for a given state and input, the transition function always produces a unique next state.

\paragraph{Formally Verified Solver} A natural extension is the implementation of a formally
verified solver. Specifically, when one of the dimensions is even, there always exists a 
\href{http://www.datagenetics.com/blog/april42013/}{Hamiltonian cycle} which the snake can follow to
guarantee a win.

\paragraph{Extended Game Rules}
The current model assumes a single food item, empty boards, and hard boundaries. The framework could
be extended to support richer game mechanics, such as multiple simultaneous food items, static
obstacles, or wrap-around borders. Each of these features would require new invariants and
additional reasoning about collision detection and food placement, providing a more challenging
verification task while preserving the same state-machine structure.

\paragraph{Scalability and Automation}
Although verification time remains reasonable for this project, the number of auxiliary lemmas
required for list manipulation and grid reasoning highlights the limits of automation. Future work
could investigate alternative encodings of the snake body or grid, or compare Stainless with other
verification tools to evaluate proof effort, expressiveness, and scalability.

\paragraph{Executable Refinement}
Finally, one could establish a refinement relation between the verified model and a real-time or
graphical implementation of the game. Proving that the executable version refines the verified
state-machine specification would strengthen the connection between formal guarantees and deployed
software.
...
\begin{itemize}
    \item Clément
        \begin{itemize}
            \item Attempts were made to use ChatGPT to generate direct proofs; however, this was not successful. ChatGPT was more useful for interpreting Stainless verification output when proofs were invalid, but it was unable to infer concrete solutions to the reported issues.
        \end{itemize}
    \item Mohamed Taha
    \begin{itemize}
        \item LLMs were helpful for frontend development. They were used to implement the dark theme and the debug mode, where snake cells are numbered.
        \item The initial WebSocket implementation (both frontend and backend) was buggy, but LLMs helped fix the issues after a few iterations.
        \item LLMs were not very useful for Stainless: they frequently hallucinated code or attempted to use Scala instead of Stainless.
        \item The presentation was partially created using AI (Copilot), which efficiently extracted relevant code blocks and integrated them into the \LaTeX{} slides. It also gave good suggestions on how our presentation related to the codebase, since it had access to it.
    \end{itemize}
    \item Ozair
        \begin{itemize}
            ...
            \item Updating the visuals of the game (which I did not write) when changing the backend
                ...
                additions/removals which worked surprisingly well.
